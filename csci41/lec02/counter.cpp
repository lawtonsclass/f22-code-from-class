...
class Counter {
  ...
  private:
    int count;
    vector<int> blah; // default constructor called automatically unless you change it
    const double d;
};
...
int main() {
  Counter c; // implicitly calling the constructor for us!
  c.inc();
  c.inc();
  c.inc();
  ...
  // when you say c + c2, the compiler tries to call both operator+(c, c2)
  // OR c.operator+(c2)
  Counter c3 = c + c2;  // secretly translated into c.operator+(c2)
  cout << c3.getCount() << endl;

  Counter c4 = c * c2; // calls operator*(c, c2)
